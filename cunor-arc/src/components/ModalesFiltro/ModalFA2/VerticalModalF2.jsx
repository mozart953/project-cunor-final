...
function MyVerticallyCenteredModalF2({setBusqueda,...props}) {
    ...
    useEffect(()=>{
      ...
      const letraS= letraSeleccionada!==''&&letraSeleccionada!==null?`&letra=${letraSeleccionada}`:'';
             fetch(`/api/datos/reDetallesTrabajoInicial/filtroAutores?page=${currentpage}&itemsPagina=${itemspagina}&idEstado=${estado}&searchTerm=${busquedaA}&orderDirection=${valorseleccionado}&orderCampo=${valorseleccionado2}${letraS}`)
    ...
    const onSubmit = async (e)=>{
      ...
      const letraS= letraSeleccionada!==''&&letraSeleccionada!==null?`&letra=${letraSeleccionada}`:'';
      const respuesta = await fetch(`/api/datos/reDetallesTrabajoInicial/filtroAutores?page=${currentpage}&itemsPagina=${itemspagina}&idEstado=${estado}&searchTerm=${busquedaA}&orderDirection=${valorseleccionado}&orderCampo=${valorseleccionado2}${letraS}`);
                            ...
                            <li className={`page-item ${currentpage === totalPaginas ? 'disabled' : ''}`}>
                              ...
                              
                              {
                                   trabajos && trabajos.length!==0?( trabajos.map((data)=>(
                                          ...
                                          <div className="card mb-4 bg-dark text-white border-secondary" style={{width:'85%', margin:'0 auto', borderWidth: '3px'}} key={data.ID_Autor}>
...
export default MyVerticallyCenteredModalF2;