...
function MyVerticallyCenteredModalC({setBusqueda,...props}) {
    ...
    useEffect(()=>{
      ...
      const letraS= letraSeleccionada!==''&&letraSeleccionada!==null?`&letra=${letraSeleccionada}`:'';
        if(props.idusuario && props.idcarrera){
            fetch(`/api/datos/reDetalleTrabajoInterno/filtroCategoriasP?idUsuario=${props.idusuario}&idCarrera=${props.idcarrera}&page=${currentpage}&itemsPagina=${itemspagina}&searchTerm=${busquedaA}&orderDirection=${valorseleccionado}&orderCampo=${valorseleccionado2}${letraS}`)
    ...
    const onSubmit = async (e)=>{
      ...
      const letraS= letraSeleccionada!==''&&letraSeleccionada!==null?`&letra=${letraSeleccionada}`:'';
        const respuesta = await fetch(`/api/datos/reDetalleTrabajoInterno/filtroCategoriasP?idUsuario=${props.idusuario}&idCarrera=${props.idcarrera}&page=${currentpage}&itemsPagina=${itemspagina}&searchTerm=${busquedaA}&orderDirection=${valorseleccionado}&orderCampo=${valorseleccionado2}${letraS}`);
      ...
      <Modal.Body       style={{ backgroundColor: 'black', color: 'white' }}>
                    <div className="mb-3 d-flex justify-content-center align-items-center">
                        <form className="input-group" style={{width: "600px"}} onSubmit={onSubmit}>
                                <input type="search" className="form-control" placeholder="Buscar categorias en este espacio" aria-label="Search" value={busquedaA} onChange={(e)=>{setBusquedaA(e.target.value)}}/>
                            ...
                            <li className={`page-item ${currentpage === totalPaginas ? 'disabled' : ''}`}>
                              ...
                              
                              {
                                   trabajos && trabajos.length!==0?( trabajos.map((data)=>(
                                          ...
                                          <div className="card mb-4 bg-dark text-white border-secondary" style={{width:'85%', margin:'0 auto', borderWidth: '3px'}} key={data.ID_Categoria}>
                                                      ...
                                                      {/* <strong>Autor:</strong>{" "} */}
                                                        ...
                                                        <div className='mt-3 mb-2'>
                                                            <button type="button" className="btn btn-success" style={{ marginLeft: '20px' }}
                                                                onClick={()=>{
                                                                    const nombreitem = `${data.nombreCategoria}`;
