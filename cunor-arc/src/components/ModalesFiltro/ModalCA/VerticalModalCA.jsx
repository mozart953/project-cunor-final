...
function MyVerticallyCenteredModalCA({setBusqueda,...props}) {
    ...
    const ordenQuery2 = [{id:1, ord:'Nombre', ordBase:'nombreCarrera'},
                         ];
    ...
    useEffect(()=>{
      //console.log("idUse: " + props.idusuario + "idCarrera " + props.idcarrera);
            ...
            fetch(`/api/datos/reDetallesTrabajoInicial/filtroCarrerasP?&page=${currentpage}&itemsPagina=${itemspagina}&idEstado=${estado}&searchTerm=${busquedaA}&orderDirection=${valorseleccionado}&orderCampo=${valorseleccionado2}${letraS}`)
    ...
    const onSubmit = async (e)=>{
      ...
      const respuesta = await fetch(`/api/datos/reDetallesTrabajoInicial/filtroCarrerasP?&page=${currentpage}&itemsPagina=${itemspagina}&idEstado=${estado}&searchTerm=${busquedaA}&orderDirection=${valorseleccionado}&orderCampo=${valorseleccionado2}${letraS}`);
      ...
      <Modal.Body       style={{ backgroundColor: 'black', color: 'white' }}>
                    <div className="mb-3 d-flex justify-content-center align-items-center">
                        <form className="input-group" style={{width: "600px"}} onSubmit={onSubmit}>
                                <input type="search" className="form-control" placeholder="Buscar carreras en este espacio" aria-label="Search" value={busquedaA} onChange={(e)=>{setBusquedaA(e.target.value)}}/>
                            ...
                            <li className={`page-item ${currentpage === totalPaginas ? 'disabled' : ''}`}>
                              ...
                              
                              {
                                   trabajos && trabajos.length!==0?( trabajos.map((data)=>(
                                          ...
                                          <div className="card mb-4 bg-dark text-white border-secondary" style={{width:'85%', margin:'0 auto', borderWidth: '3px'}} key={data.ID_Carrera}>
                                                      ...
                                                      {/* <strong>Autor:</strong>{" "} */}
                                                        ...
                                                        <div className='mt-3 mb-2'>
                                                            <button type="button" className="btn btn-success" style={{ marginLeft: '20px' }}
                                                                onClick={()=>{
                                                                    const nombreitem = `${data.nombreCarrera}`;
                                                                    setBusqueda(nombreitem);
                                                                    props.onHide();
                                                                }}
                                                            ><strong>Ver trabajos asociados</strong></button>
                                                        </div>                            
...
export default MyVerticallyCenteredModalCA;