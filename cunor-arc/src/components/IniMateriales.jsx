...
function CompoMaterialesPage({datos, totalItems}){
    ...
    useEffect(
        ()=>{
            if(currentpage){
                fetch(`/api/datos/reMaterialPaginado?page=${currentpage}&itemsPagina=${itemspagina}&searchTerm=${busqueda}`)
    ...
    const onSubmit = async (e)=>{
        ...
        const respuesta = await fetch(`/api/datos/reMaterialPaginado?page=${currentpage}&itemsPagina=${itemspagina}&searchTerm=${busqueda}`);
        ...
        <>
            <div className="card text-bg-secondary mb-3" style={{width:'50%', margin:'0 auto', borderRadius:'15px'}}>
                    <div className="card-body">
                        <legend className="text-center mb-4"><i className="bi bi-pin-angle-fill"></i><strong>Gestión de tipos de material -CUNOR-</strong></legend>                       
                    </div>
            </div>

            <div className="mb-3 d-flex justify-content-center align-items-center">
                        <form className="input-group" style={{width: "600px"}} onSubmit={onSubmit}>
                                <input type="search" className="form-control" placeholder="Buscar material" aria-label="Search" value={busqueda} onChange={(e)=>{setBusqueda(e.target.value)}}/>
...
export default CompoMaterialesPage;