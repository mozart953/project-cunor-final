...
function CompoIdiomasPage({datos, totalItems}){
    ...
    useEffect(
        ()=>{
            if(currentpage){
                fetch(`/api/datos/reIdiomasPaginado?page=${currentpage}&itemsPagina=${itemspagina}&searchTerm=${busqueda}`)
    ...
    const onSubmit = async (e)=>{
        ...
        const respuesta = await fetch(`/api/datos/reIdiomasPaginado?page=${currentpage}&itemsPagina=${itemspagina}&searchTerm=${busqueda}`);
        ...
        <>
            ...
            <div className="mb-3 d-flex justify-content-center align-items-center">
                        <form className="input-group" style={{width: "600px"}} onSubmit={onSubmit}>
                                <input type="search" className="form-control" placeholder="Buscar idioma" aria-label="Search" value={busqueda} onChange={(e)=>{setBusqueda(e.target.value)}}/>
            ...
            <div className="d-flex justify-content-between" style={{ padding: '10px 120px 10px 0px' }}>
                <div className="d-flex content-center" style={{paddingTop:'20px', paddingLeft:'100px', paddingBottom:'10px'}}>
                    ...
                    ><i className="bi bi-plus-lg"></i><strong>Agregar idioma</strong></button>
                </div>
                            ...
                            <tbody>
                                {
                                    datosG && datosG.length!==0?(datosG.map((data)=>(
                                        <tr key={data.ID_Idioma}>
                                        <th scope="row">{data.ID_Idioma}</th>
                                        <td>{data.nombre}</td>
                                        <td>{data.codigo}</td>
                                        <td>
...
export default CompoIdiomasPage;