...
function CompoFacultadesPage({datos, totalItems}){

    const router = useRouter();

    const [datosfacultad, setFacultad] = useState([]);
    ...
    useEffect(
        ()=>{
            if(currentpage){
                fetch(`/api/datos/reFacultadesPaginado?page=${currentpage}&itemsPagina=${itemspagina}&searchTerm=${busqueda}`)
                .then(data=>data.json()).then(datos=>{setFacultad(datos.items); setTotalitems(datos.total);})
            }
        },[currentpage]
    )
    ...
    const onSubmit = async (e)=>{
        ...
        const respuesta = await fetch(`/api/datos/reFacultadesPaginado?page=${currentpage}&itemsPagina=${itemspagina}&searchTerm=${busqueda}`);
        const datos = await respuesta.json();
        console.log(datos);

        if(respuesta.ok){
            setFacultad(datos.items); 
        ...
        <>
            <div className="card text-bg-secondary mb-3" style={{width:'50%', margin:'0 auto', borderRadius:'15px'}}>
                    <div className="card-body">
                        <legend className="text-center mb-4"><i className="bi bi-pin-angle-fill"></i><strong>Gestión de facultades -CUNOR-</strong></legend>                       
                    </div>
            </div>

            <div className="mb-3 d-flex justify-content-center align-items-center">
                        <form className="input-group" style={{width: "600px"}} onSubmit={onSubmit}>
                                <input type="search" className="form-control" placeholder="Buscar facultad" aria-label="Search" value={busqueda} onChange={(e)=>{setBusqueda(e.target.value)}}/>
            ...
            <div className="mt-4" style={{width:'85%', margin:'0 auto'}}>
                    <div className="content-center d-flex justify-content-center align-items-center">
                        ...
                        <table className="table table-dark table-striped text-center" style={{borderRadius: '15px', overflow: 'hidden', border:'1px solid gray'}}>
                            ...
                            <tbody>
                                {
                                    datosfacultad && datosfacultad.length!==0?(datosfacultad.map((data)=>(
                                        <tr key={data.ID_Facultad}>
                        ...
                        {
                            datosfacultad && datosfacultad.length===0?(
                                busquedainte!==""?(
...
export default CompoFacultadesPage;